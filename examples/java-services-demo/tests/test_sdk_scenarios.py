...
EXPECTED_IDS = tuple(f"SDK-{index:02d}" for index in range(1, 16))
...
def test_expected_negative_contracts_are_reported_as_passed_demonstrations() -> None:
    results = {
    ...
    for scenario_id in negative_scenarios:
        assert results[scenario_id].status is ScenarioStatus.PASSED
