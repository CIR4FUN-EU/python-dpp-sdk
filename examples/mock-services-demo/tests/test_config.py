...
from dpp_mock_services_demo.config import DemoConfig, load_config
...
def test_import_order_does_not_capture_environment_values(
    ...
    env_file = tmp_path / "pinned.env"
    _write_env(env_file)
    config_module = importlib.import_module("dpp_mock_services_demo.config")
    monkeypatch.setenv("DPP_REPO_BASE_URL", "http://localhost:18080")
...
def test_default_configuration_honors_the_demo_environment_profile_override(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile = tmp_path / "basic.env"
    _write_env(profile)
    monkeypatch.setenv("DPP_DEMO_ENV_FILE", str(profile))

    config = load_config()

    assert config.env_file == profile.resolve()
