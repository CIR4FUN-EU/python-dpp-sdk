...
from dpp_mock_services_demo.maintainer import _parser, main
from dpp_mock_services_demo.verification.runner import run_maintainer_mode
...
def test_maintainer_exposes_explicit_verification_modes() -> None:
    ...
    assert _parser().parse_args(["verify", "--summary"]).summary is True


def test_maintainer_summary_is_one_line_and_omits_scenario_details(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert main(["sdk-contracts", "--summary"]) == 0

    output = capsys.readouterr().out.strip()
    assert output.startswith("SDK:")
    assert "SDK-01" not in output
    assert "total=17" in output


def test_public_cli_help_keeps_legacy_modes_out_of_the_consumer_path() -> None:
    from dpp_mock_services_demo.__main__ import _parser as public_parser
