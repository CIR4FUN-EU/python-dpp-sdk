"""Annotated, assertion-based SDK-01 through SDK-17 capability demonstrations.

The ``sdk`` command selects SDK-01, SDK-02, SDK-03, SDK-06, and SDK-07 below.
Each small ``_sdk_NN`` function is deliberately self-contained: it makes public
SDK calls, asserts the contract being demonstrated, and returns display-ready
teaching evidence. The remaining scenarios are used by maintainer verification.
"""
...
# Keep expected-error handling uniform so every scenario proves both the error
# type and, where relevant, the public diagnostic text shown to a consumer.
def _expect(
