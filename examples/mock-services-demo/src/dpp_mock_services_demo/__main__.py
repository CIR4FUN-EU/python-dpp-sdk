"""Command-line entry point for the isolated Mock-services consumer demo."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m dpp_mock_services_demo",
        description="Exercise the public Python SDK against disposable mock service images.",
    )
    parser.add_argument(
        "mode",
        ...
        help=(
            ...
            "Legacy compatibility commands remain supported; see ADVANCED_OPERATIONS.md."
        ),
    )
...
def _summary(mode: str) -> str:
    return {
        ...
        "demo": "Curated live Mock demonstration completed",
        "full": "Full repository and registry health check completed",
        "integration": "Live educational mock-service integration demonstration completed",
        "services": "repository and registry interoperability completed",
        "all": "SDK demonstration and mock-service interoperability completed",
        "verify": "Full assertion-based SDK and Mock service verification completed",
    }[mode]


def _load_service_config(args: argparse.Namespace) -> DemoConfig:
    """Load configuration only for a mode that contacts mock services."""
...
def _report(
    ...
    *,
    ...
    if mode == "verify" and not is_legacy:
        verdict = (
            InteroperabilityVerdict.PYTHON_MOCK_SERVICES_INTEROPERABILITY_FAILED
            if failed
            else InteroperabilityVerdict.PYTHON_MOCK_SERVICES_INTEROPERABILITY_VERIFIED
        )
    else:
        verdict = InteroperabilityVerdict.PYTHON_MOCK_SERVICES_INTEROPERABILITY_INCOMPLETE
