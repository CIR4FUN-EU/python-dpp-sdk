"""Configuration for the repository-only Mock-services consumer demo."""
...
_DEMO_ENV_FILE_KEY = "DPP_DEMO_ENV_FILE"
...
def load_config(env_file: Path | None = None, *, legacy: bool = False) -> DemoConfig:
    ...
    if env_file is None:
        configured_path = os.environ.get(_DEMO_ENV_FILE_KEY, "").strip()
        path = Path(configured_path) if configured_path else _project_root() / ".env"
