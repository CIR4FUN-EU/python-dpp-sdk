# Advanced mock-services operations and troubleshooting

The [demo README](README.md) is the normal consumer path: install, offline
`sdk`, connected `demo`, full test suites, and project-scoped cleanup. This is
an optional reference for extra lifecycle commands, alternate profiles,
troubleshooting, maintainer verification, and CI/release evidence.
...
`python -m dpp_mock_services_demo.maintainer sdk-contracts`, `live`, and `verify` for the exhaustive
...
The manually triggered GitHub workflow `Mock services demo verification` builds both wheels, starts
a fresh disposable stack for the selected profile, runs installed-wheel verification, captures
evidence, and tears down its project. The PyPI release workflow does not start Mock images; it runs
...
- **SDK development:** run the root pytest suite with `-m "not integration"`.
- **Demo development:** run the demo pytest suite with `-m "not integration"`.
- **Connected integration:** `python -m dpp_mock_services_demo.maintainer live --env-file <file>`
  runs the 22 live scenarios once.
- **Full release confidence:** use the README full-verification flow. It runs
  both non-live suites, then strict clean-wheel verification and the 22 live scenarios once.
